...
if type docker-compose > /dev/null 2>&1; then
    ...
    echo "ssssssssssssss 1"
    TARGET_COMPOSE_ARCH="$(uname -m)"
    if [ "${TARGET_COMPOSE_ARCH}" = "amd64" ]; then
        echo "ssssssssssssss 2"
        TARGET_COMPOSE_ARCH="x86_64"
    fi
    echo "ssssssssssssss 3"
    if [ "${TARGET_COMPOSE_ARCH}" != "x86_64" ]; then
        echo "ssssssssssssss 4"
        # Use pip to get a version that runns on this architecture
        if ! dpkg -s python3-minimal python3-pip libffi-dev python3-venv > /dev/null 2>&1; then
            echo "ssssssssssssss 5"
            apt_get_update_if_needed
            apt-get -y install python3-minimal python3-pip libffi-dev python3-venv
        fi
        echo "ssssssssssssss 6"
        ...
        pipx_bin=pipx
        echo "ssssssssssssss 7"
        if ! type pipx > /dev/null 2>&1; then
            echo "ssssssssssssss 8"
            pip3 install --disable-pip-version-check --no-warn-script-location  --no-cache-dir --user pipx
            pipx_bin=/tmp/pip-tmp/bin/pipx
        fi
        echo "ssssssssssssss 9"
        ${pipx_bin} install --system-site-packages --pip-args '--no-cache-dir --force-reinstall' docker-compose
        rm -rf /tmp/pip-tmp
        echo "ssssssssssssss 10"
    else
        echo "ssssssssssssss 11"
        echo "https://get.daocloud.io/docker/compose/releases/download/v2.0.1/docker-compose-$(uname -s)-${TARGET_COMPOSE_ARCH}"
        ...
        curl -fsSL "https://get.daocloud.io/docker/compose/releases/download/v2.0.1/docker-compose-$(uname -s)-${TARGET_COMPOSE_ARCH}" -o /usr/local/bin/docker-compose
